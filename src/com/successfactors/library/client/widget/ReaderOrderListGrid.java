	...
	private void updateDS(final int itemsPerPage, final int pageNum) {
		new RPCCall<OrderPage>() {
			...
			protected void callService(AsyncCallback<OrderPage> cb) {
				orderService.getOrderList(OrderStatusType.NOW, SFLibrary.get().getNowUser().getUserEmail(), itemsPerPage, pageNum, cb);
			}
		}.retry(3);
	}
	...
	public void doNextPage() {
		...
		for (Record record : this.getRecords()) {
			...
			new RPCCall<OrderPage>() {
				...
				@Override
				protected void callService(AsyncCallback<OrderPage> cb) {
					orderService.getOrderList(OrderStatusType.NOW, SFLibrary.get().getNowUser().getUserEmail(), DEFAULT_RECORDS_EACH_PAGE, pageNowNum+1, cb);
				}
			}.retry(3);
